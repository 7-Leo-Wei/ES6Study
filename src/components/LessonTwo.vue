...
a <span>// ReferenceError: a is not defined.</span>
b <span>// 1</span></pre>
			</div>
...
console.log(i);
<span>// ReferenceError: i is not defined</span></pre>
			</div>
			<p>上面代码中，计数器i只在<code>for</code>循环体内有效，在循环体外引用就会报错。</p>
...
a[6](); <span>// 6</span></pre>
			...
			<p>上面代码中，变量<code>i</code>是<code>let</code>声明的，当前的<code>i</code>只在本轮循环有效，所以每一次循环的<code>i</code>其实都是一个新的变量，所以最后输出的是<code>6</code>。你可能会问，如果每一轮循环的变量<code>i</code>都是重新声明的，那它怎么知道上一轮循环的值，从而计算出本轮循环的值？这是因为<code>JavaScript</code> 引擎内部会记住上一轮循环的值，初始化本轮的变量<code>i</code>时，就在上一轮循环的基础上进行计算。</p>
...
<span>// abc</span></pre>
			...
			<div class="code">
<pre><span>// var 的情况</span>
console.log(foo); <span>// 输出undefined</span>
var foo = 2;

<span>// let 的情况</span>
console.log(bar); <span>// 报错ReferenceError</span>
let bar = 2;</pre>
			</div>
...
if (true) {
  tmp = 'abc'; <span>// ReferenceError</span>
  let tmp;
}</pre>
			</div>
			...
			<p>ES5明确规定，如果区块中存在<code>let</code>和<code>const</code>命令，这个区块对这些命令声明的变量，从一开始就形成了封闭作用域。凡是在声明之前就使用这些变量，就会报错。</p>
			<p>总之，在代码块内，使用<code>let</code>命令声明变量之前，该变量都是不可用的。这在语法上，陈伟“暂时性死区”（temporal dead zone，简称TDZ）。</p>
			<div class="code">
<pre>if (true) {
  <span>// TDZ开始</span>
  tmp = 'abc'; <span>// ReferenceError</span>
  console.log(tmp); <span>// ReferenceError</span>

  let tmp; <span>// TDZ结束</span>
  console.log(tmp); <span>// undefined</span>

  tmp = 123;
  console.log(tmp); <span>// 123</span>
}</pre>
			</div>
			<p>上面代码中，在<code>let</code>命令声明变量<code>tmp</code>之前，都属于变量<code>tmp</code>的“死区”，</p>
			<p>“暂时性死区”也意味着<code>typeof</code>不再是一个百分之百安全的操作。</p>
			<div class="code">
<pre>typeof x; <span>// ReferenceError</span>
let x;</pre>
			</div>
			...
			<p>作为比较，如果一个变了根本没有被声明，使用<code>typeof</code>反而不会报错。</p>
			<div class="code">
				<pre>typeof undeclared_variable <span>// "undefined"</span></pre>
			</div>
...
bar(); <span>// 报错</span></pre>
			...
			<div class="code">
<pre>function bar(x = 2, y = x) {
  return [x, y];
}
bar(); <span>// [2, 2]</span></pre>
			</div>
...
<span>// 报错</span>
let x = x;
<span>// ReferenceError: x is not defined</span></pre>
			</div>
...
f();  <span>// undefined</span>
</pre>
			</div>
			<p>上面代码的愿意是，<code>if</code>代码块的外部使用外层的<code>tmp</code>变量，内部使用内层的<code>tmp</code>变量。但是，函数<code>f</code>执行后，输出结果为<code>undefined</code>，原因在于变量提升，导致内层的<code>tmp</code>变量覆盖了外层的<code>tmp</code>变量。</p>
			<p>第二种场景，用来计数的循环变量泄漏为全局变量。</p>
			<div class="code">
<pre>
var s = 'hello';
...
console.log(i);  <span>// 5</span>
</pre>
			</div>
			<p>上面代码中，变量<code>i</code>只用来控制循环，但是循环结束后，它并没有消失，泄漏成了全局变量。</p>
			<h4>ES6的块级作用域</h4>
...
function f1() {
	...
	console.log(n);  <span>// 5</span>
}
</pre>
			</div>
			<p>上面的函数有两个代码块，都声明了变量n，运行后输出5,。这表示外层代码块不受内层代码块的影响。如果两次都使用<code>var</code>定义变量<code>n</code>，最后输出的值才是10。</p>
			<p>ES6 允许块级作用域的任意嵌套。</p>
			<div class="code">
<pre>{{{{
  {let insane = 'Hello World'}
  console.log(insane); <span>// 报错</span>
}}}};</pre>
			</div>
...
<pre>{{{{
  let insane = 'Hello World';
  {let insane = 'Hello World'}<span></span>
}}}};</pre>
			</div>
			<p>块级作用域的出现，实际上使得获得广泛应用的匿名立即执行函数表达式（匿名 IIFE）不再必要了。</p>
			<div class="code">
<pre><span>// IIFE 写法</span>
(function () {
  var tmp = ...;
  ...
}());
...
try {
  ...
  <span>// ...</span>
			...
			<div class="code">
<pre>function f() { console.log('I am outside!'); }
...
<span>// Uncaught TypeError: f is not a function</span></pre>
			</div>
...
<span>// Uncaught TypeError: f is not a function</span></pre>
			...
			<div class="code">
<pre><span>// 块级作用域内部的函数声明语句，建议不要使用</span>
{
  let a = 'secret';
  function f() {
    return a;
...
PI = 3;
<span>// TypeError: Assignment to constant variable.</span></pre>
			</div>
			<p>上面代码表明改变常量的值会报错。</p>
			<p><code>const</code>声明的变量不得改变值，这意味着，<code>const</code>一旦声明变量，就必须立即初始化，不能留到以后赋值。</p>
			<div class="code">
<pre>const foo;
<span>// SyntaxError: Missing initializer in const declaration</span></pre>
			</div>
...
MAX <span>// Uncaught ReferenceError: MAX is not defined</span></pre>
			</div>
			<p><code>const</code>命令声明的常量也是不提升，同样存在暂时性死区，只能在声明的位置后面使用。</p>
			<div class="code">
<pre>if (true) {
  console.log(MAX); <span>// ReferenceError</span>
  const MAX = 5;
}</pre>
			</div>
...
<span>// 为 foo 添加一个属性，可以成功</span>
foo.prop = 123;
foo.prop <span>// 123</span>

<span>// 将 foo 指向另一个对象，就会报错</span>
foo = {}; <span>// TypeError: "foo" is read-only</span></pre>
			</div>
			<p>上面代码中，常量<code>foo</code>储存的是一个地址，这个地址指向一个对象。不可变的只是这个地址，即不能把<code>foo</code>指向另一个地址，但对象本身是可变的，所以依然可以为其添加新属性。</p>
			<p>下面是另一个例子。</p>
			<div class="code">
<pre>const a = [];
a.push('Hello'); <span>// 可执行</span>
a.length = 0;    <span>// 可执行</span>
a = ['Dave'];    <span>// 报错</span></pre>
			</div>
...
<span>// 常规模式时，下面一行不起作用；</span>
<span>// 严格模式时，该行会报错</span>
foo.prop = 123;</pre>
			</div>
...
window.a <span>// 2</span></pre>
			...
			<div class="code">
<pre>var a = 1;
<span>// 如果在 Node 的 REPL 环境，可以写成 global.a</span>
<span>// 或者采用通用方法，写成 this.a</span>
window.a <span>// 1</span>

let b = 1;
window.b <span>// undefined</span></pre>
			</div>
